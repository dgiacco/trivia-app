...
const Modal = ({ isFinalResult, count, totalQuestions }: ModalProps) => {
  const { setSelectedAnswers } = useGlobalContext();
          ...
          <div className="bg-white p-7 lg:p-10">
            {isFinalResult ? (
              <>
                ...
                <p className={modalText}>{finalMessage}</p>
                <p className={modalText}>Your score is:</p>
                <h2 className={modalCounter}>
                  {count} / {totalQuestions}
                </h2>
              </>
            ) : (
              <>
                <h1 className={modalTitle}>No Questions Available 😞</h1>
                <p className={modalText}>
                  There are no questions available for this combination.
                </p>
                <p className={modalText}>Please try another one.</p>
              </>
            )}
          </div>
          <div
            className={
              isFinalResult ? finalResultContainer : noQuestionsContainer
            }
          >
            {isFinalResult && (
              <Button onClick={goToReview} variant="secondary">
                Review
              </Button>
            )}

            <Button onClick={goToMenu}>
              {isFinalResult ? "Play again!" : "Go back!"}
            </Button>
          </div>
        </div>
      </div>
